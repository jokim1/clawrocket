...
import { shrinkHistoryToBudget } from './history-budget.js';
...
describe('main-executor (pure)', () => {
  ...
  it('bounds direct-path main history by model context window', async () => {
    getDb()
      .prepare(
        `
      ...
      .run(6900, 'provider.anthropic', 'claude-sonnet-4-6');
    ...
    await executeMainChannel(
      {
        runId: 'run-main-budget',
        threadId: 'thread-budget',
        requestedBy: 'owner-1',
        triggerMessageId: 'msg-budget-4',
        triggerContent: contents[4],
      },
      new AbortController().signal,
    );

    expect(executeWithAgent).toHaveBeenCalledTimes(1);
    const context = vi.mocked(executeWithAgent).mock.calls[0]?.[1];
  ...
  it('passes no direct-path history when reserves exhaust the budget', async () => {
    getDb()
      .prepare(
        `
        UPDATE llm_provider_models
      ...
      .run(5000, 'provider.anthropic', 'claude-sonnet-4-6');

    const now = new Date().toISOString();
    createMessage({
      id: 'msg-zero-budget',
      talkId: null,
      threadId: 'thread-zero-budget',
      role: 'user',
      content: 'This should be trimmed away',
      createdBy: 'owner-1',
      createdAt: now,
    });

    vi.mocked(executeWithAgent).mockResolvedValue({
      content: 'No history reply',
      agentId: 'agent.main',
      providerId: 'provider.anthropic',
      modelId: 'claude-sonnet-4-6',
      usage: {
    ...
    await executeMainChannel(
      {
        runId: 'run-main-zero-budget',
        threadId: 'thread-zero-budget',
        requestedBy: 'owner-1',
        triggerMessageId: 'msg-zero-budget',
        triggerContent: 'This should be trimmed away',
      },
      new AbortController().signal,
    );
  ...
  it('uses the extracted shared history-budget helper', () => {
    expect(
    ...
    ).toEqual([{ role: 'user', content: 'c'.repeat(10) }]);
  });
