...
function buildApp(opts: WebServerOptions): Hono {
  ...
  app.post('/api/v1/browser/setup', async (c) => {
    ...
    const payload = parseJsonPayload<Record<string, unknown>>(await c.req.text());
    if (!payload.ok) {
      return c.json(
        { ok: false, error: { code: 'invalid_json', message: payload.error } },
        400,
  ...
  app.post('/api/v1/browser/confirmations/:confirmationId/approve', async (c) => {
    const auth = requireAuth(c);
    if (!auth) return unauthorized(c);
    const rateResult = checkRateLimit({
      principalId: auth.userId,
  ...
  app.post('/api/v1/browser/confirmations/:confirmationId/reject', async (c) => {
    const auth = requireAuth(c);
    if (!auth) return unauthorized(c);
    const rateResult = checkRateLimit({
      principalId: auth.userId,
