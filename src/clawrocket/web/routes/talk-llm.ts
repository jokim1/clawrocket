...
function validateTalkLlmSettingsPayload(input: PutTalkLlmSettingsInput):
  ...
  if (
    typeof input.defaultRouteId !== 'string' ||
    !input.defaultRouteId.trim()
  ) {
  ...
  for (const rawProvider of input.providers) {
    ...
    if (
      ...
      return {
        ok: false,
        error: 'provider entries are missing required fields',
      };
    }
    ...
    if (provider.credential === null) {
      ...
      if (typeof secret.apiKey !== 'string' || !secret.apiKey.trim()) {
        return {
          ok: false,
          error: 'provider credentials must include apiKey',
        } as const;
      }
      credential = {
        apiKey: secret.apiKey.trim(),
        organizationId:
          typeof secret.organizationId === 'string' &&
  ...
  for (const rawRoute of input.routes) {
    ...
    if (
      steps.length === 0 ||
      steps.some((step) => !step.providerId || !step.modelId)
    ) {
      return {
        ok: false,
        error: 'each route must have at least one valid step',
      };
    }
    routes.push({
      id: route.id.trim(),
...
export function getTalkLlmSettingsRoute(input: { auth: AuthContext }): {
  statusCode: number;
